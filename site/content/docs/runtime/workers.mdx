...
The worker entry is TypeScript here for free — Nub transpiles it like any other entry. The `import.meta.resolve("./worker.ts")` call resolves the path against the current module — the clean, standard spelling. If you bundle for production, switch to `new URL("./worker.ts", import.meta.url)`, which third-party bundlers trace (see [Constructor inputs](#constructor-inputs)).
